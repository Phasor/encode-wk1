...
async function main() {
  ...
  if (process.argv.length < 4) throw new Error("Delegating to address missing");
  const delegatingToAddress = process.argv[3];
  ...
  console.log(`Delegating to ${delegatingToAddress}`);

  // delegate to delegatingToAddress
  const tx = await ballotContract.delegate(delegatingToAddress);
